...
/**
 * forEachOf and whenOf, checked against the {@code dynamic()} block they replace. Both are
 * preencoded, so the bug to catch is right text at the wrong indentation. The preprocessor records
 * a sub-chain and must restore the outer recording exactly. A dynamic block writes everything at
 * render time, so its depth is always right.
 */
