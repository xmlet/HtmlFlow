...
public class HtmlViewVisitorAsyncHot extends HtmlVisitorAsync {
    ...
    public HtmlViewVisitorAsyncHot(
        boolean isIndented,
        Supplier<PreprocessingVisitorAsync> continuationSupplier
    ) {
        /**
    ...
    /** Auxiliary for clone. */
    private HtmlViewVisitorAsyncHot(
        Appendable out,
        boolean isIndented,
        Supplier<PreprocessingVisitorAsync> continuationSupplier
    ) {
    ...
    public HtmlVisitorAsync clone(boolean isIndented) {
        return new HtmlViewVisitorAsyncHot(
            out,
            isIndented,
            continuationSupplier
        );
    }
    ...
    /**
     * In order to create well-formed HTML as well as enable progressive rendering (not blocking) the
     * thread in the async consumer), we need to use continuations.
     *
     * <p>As such, this uses PreprocessingVisitorAsync to create a chain of continuations that will be
     * created and executed on each rendering of the template.
     */
    @Override
    public CompletableFuture<Void> finishedAsync(Object model) {
        PreprocessingVisitorAsync continuationProcessor =
            continuationSupplier.get();

        CompletableFuture<Void> cf = new CompletableFuture<>();
        HtmlContinuationAsyncTerminationNode terminationNode =
            new HtmlContinuationAsyncTerminationNode(cf);
        PreprocessingVisitor.HtmlContinuationSetter.setNext(
            continuationProcessor.findLast(),
            terminationNode
        );
    ...
    public <E extends Element, U> void visitDynamic(
        ...
        throw new IllegalStateException(
            "Invalid use of visitDynamic() in HtmlViewVisitorAsyncHot! This should have been" +
            " called in the continuation processor (PreprocessingVisitorAsync) "
        );
    }

    /**
     * To ensure well-formed HTML and enable asynchronous rendering of templates we rely on
     * continuations. The PreprocessingVisitorAsync builds a chain of continuations for each template
     * rendering, allowing each part of the template to execute in sequence without blocking threads
     * and ensuring that the next continuation is only executed after the current one finishes.
     *
     * <p>As such, this method is not implemented here and should be called in the continuation
     * processor instead.
     */
    @Override
