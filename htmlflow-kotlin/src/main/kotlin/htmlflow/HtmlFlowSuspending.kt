...
suspend fun <E : Element<*, *>> E.suspending(block: suspend E.() -> Unit): E {
    if (lookForRootElement() !is HtmlDoc) {
        throw IllegalStateException(
            "Illegal use of suspending. Using .suspending { elem -> ...} without a model, is only valid in HtmlDoc."
        )
    }
    block(this)
    return this
...
fun <E : Element<*, *>, M> E.suspending(block: suspend E.(M) -> Unit): E {
    ...
    if (lookForRootElement() is HtmlDoc) {
        throw IllegalStateException(
            "Illegal use of suspending in HtmlDoc. Using .suspending { elem, model -> ...} is only valid in HtmlViewAsync."
        )
    }
