...
inline var <T : Element<T, Z>, Z : Element<*, *>> T.text: T
    get() = self()
    set(value) {
        visitor.visitText(Text(self(), visitor, value))
    }
 ...
 */
inline fun <T : Element<*, *>, Z : Element<*, *>, M> Element<T, Z>.dyn(
    crossinline cons: T.(M) -> Unit
): T {
    this.dynamic<M> { elem, model -> elem.cons(model) }
    return this.self()
}
 ...
 * @param M type of the model (aka context object)
 */
inline fun <T : Element<*, *>, Z : Element<*, *>, M> Element<T, Z>.await(
    crossinline cons: T.(M, () -> Unit) -> Unit
): T {
    this.await<M> { elem, model, cb -> elem.cons(model) { cb.finish() } }
    return this.self()
}
...
/** @param M type of the model (aka context object) */
fun <M : Any?> viewSuspend(template: HtmlPage.() -> Unit) =
    viewSuspend<M>(template, isIndented = true, threadSafe = false, preEncoding = true)

fun <M : Any?> viewSuspend(
    preEncoding: Boolean,
    template: HtmlPage.() -> Unit,
): HtmlViewSuspend<M> =
    viewSuspend(template, isIndented = true, threadSafe = false, preEncoding = preEncoding)
...
fun <M : Any?> viewSuspend(
    ...
    if (preEncoding) {
        ...
        val visitor = HtmlViewVisitorSuspend(isIndented = isIndented, first = pre.first)
...
fun <M : Any?> HtmlFlow.viewSuspend(
    template: HtmlPage.() -> Unit,
    isIndented: Boolean,
    threadSafe: Boolean,
    preEncoding: Boolean,
): HtmlViewSuspend<M> = viewSuspend(template, isIndented, threadSafe, preEncoding)

fun <M : Any?> HtmlFlow.viewSuspend(template: HtmlPage.() -> Unit): HtmlViewSuspend<M> =
    viewSuspend(template, isIndented = true, threadSafe = false, preEncoding = true)
 ...
 */
private fun preprocessingSuspend(
