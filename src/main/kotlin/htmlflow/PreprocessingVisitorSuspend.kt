...
import htmlflow.continuations.HtmlContinuationSuspendableAsync
import htmlflow.continuations.HtmlContinuationSuspendableSyncCloseAndIndent
import htmlflow.continuations.HtmlContinuationSuspendableSyncDynamic
import htmlflow.continuations.HtmlContinuationSuspendableSyncStatic
import htmlflow.continuations.HtmlContinuationSuspending
...
class PreprocessingVisitorSuspend(isIndented: Boolean) : PreprocessingVisitorAsync(isIndented) {
     ...
     *
     * U is the type of the model passed to the dynamic HTML block that is the same as T in this
     * visitor. Yet, since it came from HtmlApiFaster that is not typed by the Model, then we have
     * to use another generic argument for the type of the model.
     *
     ...
     *   visitor. </U></E>
     */
    override fun <E : Element<*, *>, U> visitDynamic(
        element: E,
        dynamicHtmlBlock: BiConsumer<E, U>,
    ) {
        /** Creates an HtmlContinuation for the dynamic block. */
        val dynamicCont: HtmlContinuation =
            HtmlContinuationSuspendableSyncDynamic(
                depth,
                isClosed,
                element,
                dynamicHtmlBlock,
        ...
        /**
         * We are resolving this view for the first time. Now we just need to create an
         * HtmlContinuation corresponding to the previous static HTML, which will be followed by the
         * dynamicCont.
         */
        chainContinuationSuspendableStatic(dynamicCont)
        /**
         * We have to run newlineAndIndent to leave isClosed and indentation correct for the next
         * static HTML block.
         */
    ...
    override fun <M, E : Element<*, *>> visitAwait(
        element: E,
        asyncHtmlBlock: AwaitConsumer<E, M?>,
    ) {
        /** Creates an HtmlContinuation for a suspending block. */
        val asyncCont: HtmlContinuation =
            HtmlContinuationSuspendableAsync(
                depth,
                isClosed,
                element,
                asyncHtmlBlock,
    ...
    override fun resolve(model: Any?) {
        ...
        val staticCont: HtmlContinuation =
            HtmlContinuationSuspendableSyncStatic(staticHtml.trim { it <= ' ' }, this, null)
        last =
            if (first == null)
                staticCont.also {
    ...
    private fun chainContinuationSuspendableStatic(nextContinuation: HtmlContinuation) {
        val staticHtml = sb().substring(staticBlockIndex)
        val staticHtmlTrimmed =
            staticHtml.trim { it <= ' ' } // trim to remove the indentation from static block
        val staticCont: HtmlContinuation =
            HtmlContinuationSuspendableSyncStatic(staticHtmlTrimmed, this, nextContinuation)
        if (first == null) first = staticCont // on first visit initializes the first pointer
        else
            HtmlContinuationSetter.setNext(
                last,
                staticCont,
            ) // else append the staticCont to existing chain
        last =
            nextContinuation.next // advance last to point to the new HtmlContinuationCloseAndIndent
    }
}
