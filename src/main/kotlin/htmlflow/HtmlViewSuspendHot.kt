 ...
 * Dynamic views are unoptimized and do not store static HTML blocks, recalculating HTML on every
 * rendering. This makes them suitable for development scenarios where template changes need
...
class HtmlViewSuspendHot<M>(
    ...
    override fun setIndented(isIndented: Boolean): HtmlViewSuspendHot<M> =
        viewSuspend<M>(template, isIndented, threadSafe, false) as HtmlViewSuspendHot<M>

    override fun getVisitor(): HtmlVisitorSuspending = visitor

    override fun getName(): String = "HtmlViewSuspendHot"

    override fun threadSafe(): HtmlViewSuspendHot<M> = HtmlViewSuspendHot(template, visitor, true)

    override fun threadUnsafe(): HtmlViewSuspendHot<M> =
        HtmlViewSuspendHot(template, visitor, false)
