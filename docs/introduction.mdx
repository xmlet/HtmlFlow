---
sidebar_position: 1
description: What HtmlFlow is, why it exists, and the type-safe internal-DSL approach behind it.
...
style, with first-class support for both **Java and Kotlin**.

There is no separate template language. You build HTML directly in your host language,
where every element and attribute is statically typed.

It is an _internal DSL_: the HTML lives inside Java/Kotlin itself, so the toolchain you
already use — compiler, debugger, IDE autocompletion — works on your views. The output is
always valid HTML 5.2.
...
Each element exposes exactly the children and attributes the specification allows.
Attribute values are constrained by enumerated types wherever the spec defines a fixed set.

So invalid markup does not compile. Nesting a `<div>` directly inside an `<h1>` fails at
build time, because in that context the method does not exist.
