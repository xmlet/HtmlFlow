...
Add the **`htmlflow-datastar`** module to your project. It already depends on **`htmlflow-kotlin`**, so no additional HtmlFlow modules are required.
...
```html
<div data-signals:count="0" data-init="@get('/examples/counter/events')">
...
User interactions are defined through **`data-on:*`** attributes. HtmlFlow Datastar exposes these as
typed event builders, where the event type (**`Click`**, **`Input`**, etc.), the action, and any modifiers
are expressed through Kotlin APIs rather than string concatenation.

In this example, the input is bound to the **`search`** signal. Every input event performs a **GET**
request after a **200 ms** debounce.
...
In the following example, **`dataIndicator`** creates a reactive indicator that is referenced both by
the **`disabled`** attribute and by the click handler.
